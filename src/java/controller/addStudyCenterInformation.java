...
import java.util.*;
import java.sql.*;
import model.studycenter;import java.sql.PreparedStatement;
/**
 *
 * @author Administrator
 */
public class addStudyCenterInformation extends HttpServlet {
    ...
    catch(Exception e)
    {
      ...
      if(err==0)
      {
            try {
                ...
                ResultSet rs;
                try {
                    stmt = con.prepareStatement("insert into app.fl_studycenter values(?,?,?)");
                    stmt.setInt(1, newStudyCenter.getStudycenter_id());
                    stmt.setString(2, newStudyCenter.getStudycenter_name());
                    stmt.setString(3, newStudyCenter.getStudycenter_location());
                    stmt.executeUpdate();
                    allOK = true;
                } catch (Exception e) {
                }
...
out.println("<br> Enter name of the Study Center:<input type=\"text\" name=\"flsc_name\" value=\""+newStudyCenter.getStudycenter_name()+"\" />This must be unique!");
